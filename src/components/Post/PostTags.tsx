...
export function PostTags() {
  const { data: tags, isLoading, isError, error } = api.post.getTags.useQuery()

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
  })

  const { mutateAsync: createTagMutate } = api.post.createTag.useMutation()
  const { mutateAsync: deleteTagMutate } = api.post.deleteTag.useMutation()
