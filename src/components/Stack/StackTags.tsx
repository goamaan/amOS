...
export function StackTags() {
  const { data: tags, isLoading, isError, error } = api.stack.getTags.useQuery()

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
  })

  const { mutateAsync: createTagMutate } = api.stack.createTag.useMutation()
  const { mutateAsync: deleteTagMutate } = api.stack.deleteTag.useMutation()
  ...
  const createTag = async ({ name }: z.infer<typeof formSchema>) => {
    await createTagMutate(
      { name },
      {
        ...
        onSuccess() {
          toast.success("Stack Tag created")
        },
      },
    )
  }
  ...
  const deleteTag = async (id: string) => {
    await deleteTagMutate(
      { id },
      {
        ...
        onSuccess() {
          toast.success("Stack Tag deleted")
        },
      },
    )
  }
